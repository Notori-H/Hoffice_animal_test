...
(function () {
  ...
  function styleStartButtons() {
    const startSection = document.getElementById("start");
    if (!startSection || document.getElementById("testModeButtons")) return;

    const oldButton = Array.from(startSection.querySelectorAll("button"))
      .find(function (button) { return button.textContent.includes("테스트 시작하기"); });
    if (!oldButton) return;
  ...
  window.startTest = function (mode) {
    ...
    if (window.testMode === LEADER_MODE) {
      alert(
        "우리 회사 리더의 업무 스타일을 떠올리며 답해주세요.\n\n" +
        "※ 재미로 참여하는 예측 콘텐츠입니다.\n" +
        "※ 참여 결과는 이벤트 종료 후 '직원들이 예측한 리더 성향' 게시물로 공유될 수 있습니다."
      );

      const savedPrompt = window.prompt;
      window.prompt = function () { return "우리 회사 리더"; };
      try {
        originalStartTest();
      } finally {
  ...
  window.renderResult = function (type, scores, sharedResult) {
    ...
    if (window.testMode === LEADER_MODE) {
      ...
      if (owner) owner.textContent = "직원들이 예측한 우리 회사 리더 유형";
    }
  };
  ...
  window.finish = function () {
    ...
    if (window.testMode === SELF_MODE && window.participantName) {
  ...
  async function saveResultWithMode(type) {
    ...
    const payload = {
      name: window.testMode === SELF_MODE ? (window.participantName || null) : null,
      mbti: type,
      animal: result.name,
